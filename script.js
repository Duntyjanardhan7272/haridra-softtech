...
function showSuggestions(query) {
    ...
    filtered.forEach(name => {
        ...
        div.addEventListener('click', () => {
            ...
            // Find the product card with this name and scroll to it
            const productCards = document.querySelectorAll('.product-card h3');
            for (const card of productCards) {
                if (card.textContent === name) {
                    card.closest('.product-card').scrollIntoView({
                        behavior: 'smooth',
                        block: 'center'
                    });
                    
                    // Add highlight effect
                    const productElement = card.closest('.product-card');
                    productElement.style.boxShadow = '0 0 0 3px rgba(231, 76, 60, 0.5)';
                    setTimeout(() => {
